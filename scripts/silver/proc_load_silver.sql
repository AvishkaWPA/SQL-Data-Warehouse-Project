...
Actions Performed:
    - Truncates Silver layer tables before loading.
    - Extracts data from Bronze layer tables.
    - Cleanses and transforms the data.
    - Loads the transformed data into Silver layer tables.
...
BEGIN
	...
	BEGIN TRY
		...
		CAST (prd_start_dt AS DATE) AS prd_start_dt, -- data transformation in type
		...
		END AS sls_due_dt,
		CASE WHEN sls_sales IS NULL OR sls_sales <=0 OR sls_sales != sls_quantity * ABS(sls_price) --handling missing data 
